...
async def freeze_membership(
    callback: CallbackQuery, callback_data: FreezeRequestCallbackFactory, bot: Bot, state: FSMContext
):
    request = callback_data
    ...
    await callback.message.answer(
        _("FREEZE_MEMBERSHIP_ok_admin").format(unfreeze_date=format_date(date=unfreeze_date, locale=data.get("locale")))
    )
    await bot.send_message(
        chat_id=request.chat_id,
        text=_("FREEZE_MEMBERSHIP_ok_member").format(unfreeze_date=format_date(date=unfreeze_date, locale=data.get("locale"))),
    )
    await callback.answer()
